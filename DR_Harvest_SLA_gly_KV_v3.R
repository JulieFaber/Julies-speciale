...
par(mfrow=c(1,1))
#plotting the model with all data
plot(drm_Harvest1_SLA,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="HA_LAR  ",ylim = c(0,200),lty=1)
...
#Fitting a dose-response model to data
par(mfrow=c(1,1))
plot(drm_Harvest1_SLA,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="HA_LAR  ",ylim = c(0,200),lty=1,pch = 20)
title(main="Papaver rhoeas")
...
write.csv2(table1$coefficients, "SLA_gly_KV1_Parameter estimates.csv")
write.csv2(table2$coefficients, "SLA_gly_KV2_Parameter estimates.csv")
write.csv2(table3$coefficients, "SLA_gly_KV3_Parameter estimates.csv")
...
write.csv2(table1, "SLA_gly_KV1_Effective dose w confint.csv")
write.csv2(table2, "SLA_gly_KV2_Effective dose w confint.csv")
write.csv2(table3, "SLA_gly_KV3_Effective dose w confint.csv")
