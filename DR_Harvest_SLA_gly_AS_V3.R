...
dat$Harvest1_SLA <- dat$Harvest1_leaf/dat$Harvest1_Total_shoot_l.s.b.f
dat$Harvest2_SLA <- numeric(n)
dat$Harvest2_SLA <- dat$Harvest2_leaf/dat$Harvest2_Total_shoot_l.s.b.f
dat$Harvest3_SLA <- numeric(n)
dat$Harvest3_SLA <- dat$Harvest3_leaf/dat$Harvest3_Total_shoot_l.s.b.f
...
plot(dat2$Dose,dat2$Harvest1_SLA , ylim = c(0,1),col="black") # no data to plot, no plant to spray, not inlcude in i exp.
plot(dat2$Dose,dat2$Harvest2_SLA , ylim = c(0,1),col="blue")# no data to plot, no plant to spray, not inlcude in i exp.
plot(dat2$Dose,dat2$Harvest3_SLA , ylim = c(0,1),col="red")
...
drm_Harvest1_SLA  <- drm(Harvest1_SLA ~ Dose2, data=dat2,fct=LL.4())
drm_Harvest2_SLA  <- drm(Harvest2_SLA ~ Dose2, data=dat2,fct=LL.4())
drm_Harvest3_SLA  <- drm(Harvest3_SLA ~ Dose2, data=dat2,fct=LL.4())
...
plot(drm_Harvest1_SLA ,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="SLA")
title(main="Sinapis arvensis", sub="(day 8)")
plot(drm_Harvest2_SLA,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="SLA")
title(main="Sinapis arvensis", sub="(day 15)")
plot(drm_Harvest3_SLA ,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="SLA")
...
plot(drm_Harvest1_SLA,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="SLA  ",ylim = c(0,1),lty=1)
title(main="Sinapis arvensis")
plot(drm_Harvest2_SLA,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="SLA  ",add = TRUE,col="red")
plot(drm_Harvest3_SLA,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="SLA  ",add = TRUE,col="blue")

grid()

#Fitting a dose-response model to data
par(mfrow=c(1,1))
plot(drm_Harvest1_SLA,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="SLA  ",ylim = c(0,1),lty=1,pch = 20)
title(main="Sinapis arvensis")
plot(drm_Harvest3_SLA,broken=TRUE,type = "all", xlab="Glyphosate (g a.i.ha^-1)", ylab="SLA  ",add = TRUE,col="blue",pch = 20)
...
write.csv2(table1$coefficients, "SLA_gly_AS1_Parameter estimates.csv")
write.csv2(table2$coefficients, "SLA_gly_AS2_Parameter estimates.csv")
write.csv2(table3$coefficients, "SLA_gly_AS3_Parameter estimates.csv")
...
write.csv2(table1, "SLA_gly_AS1_Effective dose w confint.csv")
write.csv2(table2, "SLA_gly_AS2_Effective dose w confint.csv")
write.csv2(table3, "SLA_gly_AS3_Effective dose w confint.csv")
